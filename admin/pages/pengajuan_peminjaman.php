...
// Proses jika ada aksi
if (isset($_GET['aksi']) && isset($_GET['id'])) {
    $id = $_GET['id'];
    $aksi = $_GET['aksi'];

    if ($aksi === 'terima') {
        mysqli_query($conn, "UPDATE tb_pengajuan_peminjaman SET status='diterima' WHERE id_pengajuan='$id'");
    } elseif ($aksi === 'tolak') {
        mysqli_query($conn, "UPDATE tb_pengajuan_peminjaman SET status='ditolak' WHERE id_pengajuan='$id'");
    }
...
// Ambil data pengajuan beserta nama pengguna
$data = mysqli_query($conn, "SELECT p.*, u.nama FROM tb_pengajuan_peminjaman p 
...
function formatTanggal($tanggal)
{
    ...
    $formatter = new IntlDateFormatter(
        'id_ID', // Lokalisasi bahasa Indonesia
            ...
            <ul class="space-y-2 font-medium">
                ...
                <li>
                    <a href="./pengguna.php" class="flex items-center p-2 text-gray-900 rounded-lg hover:bg-gray-100 -gray-700 group">
                        <svg class="shrink-0 w-5 h-5 text-gray-500 transition duration-75 -400 hover-group:text-gray-900 :text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 18">
                ...
                <li>
                    <a href="./pengajuan_peminjaman.php" class="flex items-center p-2 text-gray-900 rounded-lg  bg-gray-100 -gray-700 group">
                        <i class="fas fa-handshake w-5 h-5 text-gray-500 transition duration-75 text-gray-900"></i>
        ...
        <div class="p-4 mt-14">
            <div class="w-full p-4 bg-white border border-gray-200 rounded-lg shadow-sm sm:p-8 dark:bg-gray-800 dark:border-gray-700">
                <div class="mb-6 flex justify-between">
                    <h2 class="font-bold text-2xl">Manajemen Pengajuan Peminjaman</h2>
                </div>
                            ...
                            ?>
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        <?php else : ?>
                                            <button onclick="setModalData(<?= $row['id'] ?>, 'terima')"
                                                data-modal-target="confirmModal" data-modal-toggle="confirmModal"
                                                class="cursor-pointer bg-green-600 text-white px-3 py-1 rounded text-sm">Terima</button>
    
                                            <button onclick="setModalData(<?= $row['id'] ?>, 'tolak')"
                                                data-modal-target="confirmModal" data-modal-toggle="confirmModal"
                                                class="cursor-pointer bg-red-600 text-white px-3 py-1 rounded text-sm">Tolak</button>
                                        <?php endif ?>
                                    </td>
                                </tr>
                            <?php endwhile ?>
    ...
    <div id="confirmModal" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative w-full max-w-md max-h-full">
            <div class="relative bg-white rounded-lg shadow p-6">
                ...
                <form method="post" action="../backend/pengajuan_peminjaman.php">
    ...
    <script>
        function setModalData(id, action) {
            ...
            if (action === 'hapus') {
                ...
                submitButton.className = 'bg-red-600 text-white px-4 py-2 rounded cursor-pointer';
            } else if (action === 'reset') {
                title.textContent = 'Reset Password';
                message.textContent = 'Apakah Anda yakin ingin me-reset password ke default?';
                submitButton.name = 'reset';
                submitButton.className = 'bg-gray-700 text-white px-4 py-2 rounded cursor-pointer';
            } else if (action === 'terima') {
                title.textContent = 'Terima Pengajuan';
                message.textContent = 'Apakah Anda yakin ingin menerima pengajuan ini?';
                submitButton.name = 'terima';
                submitButton.className = 'bg-green-600 text-white px-4 py-2 rounded cursor-pointer';
            } else if (action === 'tolak') {
                title.textContent = 'Tolak Pengajuan';
                message.textContent = 'Apakah Anda yakin ingin menolak pengajuan ini?';
                submitButton.name = 'tolak';
                submitButton.className = 'bg-red-600 text-white px-4 py-2 rounded cursor-pointer';
            }
        }
