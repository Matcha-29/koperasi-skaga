...
if (isset($_GET['edit'])) {
    $id = $_GET['edit'];
    $result = mysqli_query($conn, "SELECT * FROM tb_pengguna WHERE id = $id");
...
<body>
    ...
    <aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform -translate-x-full bg-white border-r border-gray-200 sm:translate-x-0 -800 -700" aria-label="Sidebar">
        <div class="h-full px-3 pb-4 overflow-y-auto bg-white -800">
            <ul class="space-y-2 font-medium">
                ...
                <li>
                    <a href="./pengguna.php" class="flex items-center p-2 text-gray-900 rounded-lg bg-gray-100 -gray-700 group">
                        <svg class="shrink-0 w-5 h-5 text-gray-500 transition duration-75 -400 text-gray-900 :text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 18">
                ...
                <li>
                  ...
                  <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
                    ...
                    <ul id="dropdown-example" class="hidden py-2 space-y-2">
                        ...
                        <li>
                            <a href="./simpanan_pokok.php" class="flex items-center w-full p-2 text-gray-900 transition duration-75 rounded-lg pl-11 group hover:bg-gray-100">Simpanan Pokok</a>
                        </li>
                    </ul>
                </li>
                <li>
                  ...
                  <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
                    ...
                    <ul id="pinjaman" class="hidden py-2 space-y-2">
                        <li>
                            <a href="./pinjaman_reguler" class="flex items-center w-full p-2 text-gray-900 transition duration-75 rounded-lg pl-11 group hover:bg-gray-100">Pinjaman Reguler</a>
                        </li>
                        <li>
        ...
        <div class="p-4 mt-14">
            <div class="w-full p-4 bg-white border border-gray-200 rounded-lg shadow-sm sm:p-8 dark:bg-gray-800 dark:border-gray-700">
                <div class="mb-6 flex justify-between">
                    <h2 class="font-bold text-2xl">Master Data Pengguna</h2>

                    <button class="px-4 py-2 rounded-md bg-blue-500 font-medium text-white cursor-pointer" data-drawer-target="drawer-right-example" data-drawer-show="drawer-right-example" data-drawer-placement="right" aria-controls="drawer-right-example">Tambah Pengguna</button>
                </div>
                ...
                <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            <?php
                            $no = 1;
                            $data = mysqli_query($conn, "SELECT * FROM tb_pengguna ORDER BY nama ASC");
                            ...
                            ?>
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    <td class="px-6 py-4">
                                        <a href="?edit=<?= $row['id'] ?>" class="font-medium text-white px-4 py-2 bg-yellow-500 rounded-md cursor-pointer"><i class="fas fa-pencil-alt"></i></a>

                                        <button onclick="setModalData('<?= $row['id'] ?>', 'hapus')" data-modal-target="confirmModal" data-modal-toggle="confirmModal"
                                            class="font-medium text-white px-4 py-2 bg-red-600 rounded-md cursor-pointer"><i class="fas fa-trash"></i></button>

                                        <!-- Tombol reset password -->
                                        <button onclick="setModalData('<?= $row['id'] ?>', 'reset')" data-modal-target="confirmModal" data-modal-toggle="confirmModal"
                                            class="font-medium text-white px-4 py-2 bg-gray-700 rounded-md cursor-pointer"><i class="fas fa-sync-alt"></i></button>
                                    </td>
                                </tr>
                            <?php endwhile ?>
                        </tbody>
    ...
    <?php if ($editData): ?>
        <div class="absolute overlay inset-0 z-30 bg-gray-900/50"></div>
    <?php endif; ?>

    <!-- drawer component -->
    <div id="drawer-right-example" class="pt-20 fixed top-0 right-0 z-40 h-screen p-4 overflow-y-auto transition-transform <?= $editData ? 'translate-x-0' : 'translate-x-full' ?> bg-white w-80 dark:bg-gray-800" tabindex="-1" aria-labelledby="drawer-right-label">
        <div class="flex justify-between">
            <h5 id="drawer-right-label" class="inline-flex items-center mb-4 text-base font-semibold text-gray-500 dark:text-gray-400"><?= $editData ? 'Ubah' : 'Tambah' ?> Pengguna</h5>
        ...
        <form method="POST" action="../backend/pengguna.php" class="mb-6" enctype="multipart/form-data">
            <?php if ($editData): ?>
            ...
            <div class="mb-6">
                ...
                <input type="text" name="nama" id="nama" value="<?= $editData['nama'] ?? '' ?>" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required />
            </div>
            <div class="mb-6">
                <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Email</label>
                <input type="email" name="email" id="email" value="<?= $editData['email'] ?? '' ?>" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required />
            </div>
            <div class="mb-6">
                <label for="alamat" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Alamat</label>
                <textarea id="alamat" name="alamat" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"><?= $editData['alamat'] ?? '' ?></textarea>
            </div>
            <div class="mb-6">
                <label for="no_telepon" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">No. Telepon</label>
                <input type="number" name="no_telepon" value="<?= $editData['no_telepon'] ?? '' ?>" id="no_telepon" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required />
            </div>
            <div class="mb-6">
                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="file_input">Foto Profil</label>
                <input name="foto_profil" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" aria-describedby="file_input_help" id="file_input" type="file">
            </div>

            <?php if ($editData) : ?>
                <button name="edit" type="submit" class="cursor-pointer text-white justify-center flex items-center bg-blue-700 hover:bg-blue-800 w-full focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
                    Ubah
                </button>
            <?php else : ?>
